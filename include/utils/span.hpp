...
namespace HEXA_UTILS_NAMESPACE
{
    ...
    class Span : public SpanBase<T, Span<T>>
    {
      public:
        ...
        constexpr Span(const std::vector<T, Alloc>& vec) : Base(vec.data(), vec.size())
        {
        }

        template <class Alloc>
        constexpr Span(const Utils::vector<T, Alloc>& vec) : Base(vec.data(), vec.size())
        {
        }

        template <class Alloc>
        constexpr Span(const Utils::static_vector<T, Alloc>& vec) : Base(vec.data(), vec.size())
        {
        }

        constexpr Span(std::vector<T>& vec) : Base(vec.data(), vec.size()) {}
        constexpr Span(const Utils::vector<T>& vec) : Base(vec.data(), vec.size()) {}
        constexpr Span(const Utils::static_vector<T>& vec) : Base(vec.data(), vec.size()) {}
    ...
    class ConstSpan : public SpanBase<const T, ConstSpan<T>>
    {
      public:
        ...
        template <class U, class Alloc = class Utils::vector<U>::allocator_type>
        constexpr ConstSpan(const Utils::vector<U, Alloc>& vec) : Base(vec.data(), vec.size())
        {
        }

        template <class U, class Alloc = class Utils::static_vector<U>::allocator_type>
        constexpr ConstSpan(const Utils::static_vector<U, Alloc>& vec) : Base(vec.data(), vec.size())
        {
        }
...
{
    template <typename T>
    struct hash<Utils::Span<T>>
    {
        size_t operator()(const Utils::Span<T>& span) const noexcept
        {
            return static_cast<size_t>(span.hash());
        }
    };
    ...
    template <typename T>
    struct hash<Utils::ConstSpan<T>>
    {
        size_t operator()(const Utils::ConstSpan<T>& span) const noexcept
        {
            return static_cast<size_t>(span.hash());
        }
    };
    ...
    template <>
    struct hash<Utils::StringSpan>
    {
        size_t operator()(const Utils::StringSpan& span) const noexcept
        {
            return static_cast<size_t>(span.hash());
        }
    };
